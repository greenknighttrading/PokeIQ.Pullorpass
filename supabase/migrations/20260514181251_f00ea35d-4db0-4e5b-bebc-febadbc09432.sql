...
AS $function$
  SELECT 
    ...
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ms.price_change_7d)
      FILTER (WHERE ms.price_change_7d IS NOT NULL AND ms.price_change_7d <> 0) AS median_7d,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ms.price_change_30d)
      FILTER (WHERE ms.price_change_30d IS NOT NULL AND ms.price_change_30d <> 0) AS median_30d,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ms.price_change_90d)
      FILTER (WHERE ms.price_change_90d IS NOT NULL AND ms.price_change_90d <> 0) AS median_90d
