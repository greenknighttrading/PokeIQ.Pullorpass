...
-- 1) Remove authenticated INSERT on buylist_settings to eliminate admin bootstrap race.
--    Service role retains full access; admin can still UPDATE existing rows.
DROP POLICY IF EXISTS "Admin can insert settings" ON public.buylist_settings;

-- 2) Pin search_path on classify_set_era (function search path mutable warning)
